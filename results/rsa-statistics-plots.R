...
if(plotType == "all_brute") {
    ...
    gg1 <- gg1 + geom_line(aes(y = gen_key, color = "Keys generation"))
    gg1 <- gg1 + geom_line(aes(y = encrypt, color = "Encrypt"))
    gg1 <- gg1 + geom_line(aes(y = decrypt, color = "Decrypt"))
    #Broke
    gg2 <- ggplot(allDataMerged, aes(key_size))
    gg2 <- gg2 + ggtitle("2) Keys breaking")
    gg2 <- gg2 + xlab("Key size (bits)")
    gg2 <- gg2 + ylab("Time (s)")
    ...
    gg2 <- gg2 + geom_line(aes(y = brute_force_mean_brute, color = "Brute Force"))
    gg2 <- gg2 + geom_line(aes(y = brute_force_mean_pollard, color = "Pollard's Rho"))
